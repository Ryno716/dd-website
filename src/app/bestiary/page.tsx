...
const bestiaryEntries = [
  {
    ...
    description: "Spins tales, drops dice, and sings of epic fails and glorious crits.",
  ...
  },
  {
    ...
    description: "Submitted by legendary fan @DiceGoblin. Who’s a good beastie?",
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-yellow-100/60 to-zinc-900/80 py-12 px-4 flex flex-col items-center font-serif">
      ...
      <p className="text-lg mb-10 text-zinc-800 max-w-2xl text-center italic">
        A living tome of heroes, creatures, legendary blunders, and magical moments from our campaign and community.
      </p>
      <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 max-w-6xl w-full">
        {bestiaryEntries.map((entry) => (
          <BestiaryCard key={entry.id} entry={entry} onClick={() => setSelected(entry)} />
        ))}
      </div>
...
function BestiaryCard({ entry, onClick }) {
  return (
    <div
    ...
    >
      ...
      <div className="text-xl font-bold text-yellow-800 mb-1 text-center">{entry.name}</div>
      <div className="text-zinc-800 text-sm mb-2 text-center">{entry.description}</div>
      <div className="flex gap-2 flex-wrap justify-center">
        {entry.tags.map((tag, i) => (
          <span key={i} className="bg-yellow-300/70 text-yellow-900 rounded px-2 py-0.5 text-xs font-semibold">{tag}</span>
        ))}
      </div>
    </div>
  );
...
function BestiaryModal({ entry, onClose }) {
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-2xl font-bold text-yellow-900 mb-2">{entry.name}</div>
          <div className="mb-2 text-yellow-700 font-semibold">{entry.type}</div>
          <div className="text-zinc-800 mb-3 text-center">{entry.description}</div>
          <div className="flex gap-2 flex-wrap justify-center">
            {entry.tags.map((tag, i) => (
              <span key={i} className="bg-yellow-300/70 text-yellow-900 rounded px-2 py-0.5 text-xs font-semibold">{tag}</span>
            ))}
